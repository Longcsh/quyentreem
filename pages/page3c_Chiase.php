...
// Tổng số bài viết và tổng số trang (hiển thị tối đa 4 nút trang)
$totalDocs  = $bv->countBaiViet('chia_se');
$totalPages = max(4, ceil($totalDocs / $limit));

// Lấy bài viết theo trang
$articles = $bv->getBaiVietPhanTrang('chia_se', $limit, $offset);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['noi_dung'])) {
    ...
    $bv->themBinhLuan('chia_se', $ten, $email, $noi_dung);
    echo "<script>window.location.href='" . $_SERVER['REQUEST_URI'] . "';</script>";
exit;
}

// 💬 Lấy danh sách bình luận
$comments = $bv->getBinhLuan('chia_se');
?>

<link rel="stylesheet" href="css/content_pages.css">

<div class="page-content">
  <h2>CHIA SẺ CÂU CHUYỆN</h2>
  ...
  <div class="pagination">
    <?php if ($page > 1): ?>
      <a href="user.php?page=chiase&p=<?= $page - 1 ?>" class="prev">« Trước</a>
    <?php endif; ?>
    ...
    <?php
      ...
      for ($i = $start; $i <= $end; $i++):
    ?>
      <a href="user.php?page=chiase&p=<?= $i ?>" class="<?= $i == $page ? 'active' : '' ?>">
        <?= $i ?>
      </a>
    <?php endfor; ?>

    <?php if ($page < $totalPages): ?>
      <a href="user.php?page=chiase&p=<?= $page + 1 ?>" class="next">Sau »</a>
    <?php endif; ?>
  </div>
</div>
