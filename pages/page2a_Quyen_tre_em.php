...
$limit = 6; // số tài liệu mỗi trang
$offset = ($page - 1) * $limit;

// Tổng số tài liệu và số trang (luôn hiển thị 4 trang)
$totalDocs  = $tl->countTaiLieu('quyen_tre_em');
$totalPages = max(4, ceil($totalDocs / $limit));

// Lấy dữ liệu theo trang
$documents = $tl->getTaiLieuPhanTrang('quyen_tre_em', $limit, $offset);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['noi_dung'])) {
    ...
    $tl->themBinhLuan('quyen_tre_em', $ten, $email, $noi_dung);
...
// 💬 Lấy bình luận
$comments = $tl->getBinhLuan('quyen_tre_em');
?>
...
<div class="page-content">
  ...
  <div class="pagination">
    <?php if ($page > 1): ?>
      <a href="?page=quyentreem&p=<?= $page - 1 ?>" class="prev">« Trước</a>
    <?php endif; ?>

    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
      <a href="?page=quyentreem&p=<?= $i ?>" class="<?= $i == $page ? 'active' : '' ?>"><?= $i ?></a>
    <?php endfor; ?>

    <?php if ($page < $totalPages): ?>
      <a href="?page=quyentreem&p=<?= $page + 1 ?>" class="next">Sau »</a>
    <?php endif; ?>
  </div>
</div>
...
$(document).on("click", ".xem-tl", function() {
  ...
  $.get("BE/get_tailieu.php", { id: id }, function(res) {
    if (res.success) {
      ...
      const fileEmbed = `<p class="text-center"><a href="${fileUrl}" target="_blank" class="btn btn-primary">📥 Tải xuống tài liệu</a></p>`;
      const html = `
