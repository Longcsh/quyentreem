...
// Tổng số bài viết và tổng số trang (luôn hiển thị 4 trang)
$totalDocs  = $bv->countBaiViet('tieng_noi_cua_tre');
$totalPages = max(4, ceil($totalDocs / $limit));

// Lấy danh sách bài viết theo trang
$articles = $bv->getBaiVietPhanTrang('tieng_noi_cua_tre', $limit, $offset);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['noi_dung'])) {
    ...
    $bv->themBinhLuan('tieng_noi_cua_tre', $ten, $email, $noi_dung);
  echo "<script>window.location.href='" . $_SERVER['REQUEST_URI'] . "';</script>";
exit;
}

// 💬 Lấy bình luận
$comments = $bv->getBinhLuan('tieng_noi_cua_tre');
?>

<link rel="stylesheet" href="css/content_pages.css">

<div class="page-content">
  <h2>TIẾNG NÓI CỦA TRẺ EM</h2>
  ...
  <div class="pagination">
    <?php if ($page > 1): ?>
      <a href="user.php?page=tiengnoi&p=<?= $page - 1 ?>" class="prev">« Trước</a>
    <?php endif; ?>
    ...
    <?php
      ...
      for ($i = $start; $i <= $end; $i++):
    ?>
      <a href="user.php?page=tiengnoi&p=<?= $i ?>" 
         class="<?= $i == $page ? 'active' : '' ?>">
         <?= $i ?>
      </a>
    <?php endfor; ?>
    ...
    <?php if ($page < $totalPages): ?>
      <a href="user.php?page=tiengnoi&p=<?= $page + 1 ?>" class="next">Sau »</a>
    <?php endif; ?>
  </div>
</div>
