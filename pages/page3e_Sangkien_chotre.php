...
$totalDocs  = $bv->countBaiViet('sang_kien_cho_tre');
$totalPages = max(4, ceil($totalDocs / $limit));

// Lấy bài viết theo trang
$articles = $bv->getBaiVietPhanTrang('sang_kien_cho_tre', $limit, $offset);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['noi_dung'])) {
    ...
    $bv->themBinhLuan('sang_kien_cho_tre', $ten, $email, $noi_dung);
...
$comments = $bv->getBinhLuan('sang_kien_cho_tre');
?>

<link rel="stylesheet" href="css/content_pages.css">

<div class="page-content">
  <h2>SÁNG KIẾN CHO TRẺ</h2>
  ...
  <div class="pagination">
    <?php if ($page > 1): ?>
      <a href="user.php?page=sangkien&p=<?= $page - 1 ?>" class="prev">« Trước</a>
    <?php endif; ?>
    ...
    <?php
      ...
      for ($i = $start; $i <= $end; $i++):
    ?>
      <a href="user.php?page=sangkien&p=<?= $i ?>" class="<?= $i == $page ? 'active' : '' ?>">
        <?= $i ?>
      </a>
    <?php endfor; ?>

    <?php if ($page < $totalPages): ?>
      <a href="user.php?page=sangkien&p=<?= $page + 1 ?>" class="next">Sau »</a>
    <?php endif; ?>
  </div>
</div>
